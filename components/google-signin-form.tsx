...
/** Google is the only provider, so signing in and signing up are the same POST. */
export default function GoogleSignInForm({ redirectTo, label, disabled = false }: { redirectTo: string; label: string; disabled?: boolean }) {
  return (
    <form action={signInWithGoogle}>
      <input type="hidden" name="redirectTo" value={redirectTo} />
      <button className="google-btn" type="submit" disabled={disabled}>
        <GoogleMark />
        {label}
      </button>
    </form>
